...
const createVChat = async (vchat) => {
  try {
    const createdVchat = await db.one(
      "INSERT INTO vchats (user_id, video_url, schedule_time, duration, archive_link, start_time, end_time, archive_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",
      [
        vchat.user_id,
        vchat.video_url,
        vchat.schedule_time,
...
const updateVChat = async (id, vchat) => {
  try {
    ...
    const updatedVchat = await db.one(
      "UPDATE vchats SET user_id=$1, video_url=$2, schedule_time=$3, duration=$4, archive_link=$5, start_time=$6, end_time=$7, archive_url=$8 WHERE id=$9 RETURNING *",
      [
        user_id,
        video_url,
        schedule_time,
