...
const createUserGroup = async (userGroup) => {
    try {
        const createdUserGroup = await db.one("INSERT INTO user_groups (user_id, group_id, join_date) RETURNING*", [userGroup.user_id, userGroup.userGroup_id, userGroup.join_date])
...
const deleteUserGroup = async (id) => {
    try {
        const deletedUserGroup = await db.one("DELETE FROM user_groups WHERE id=$1 RETURNING *", id);
